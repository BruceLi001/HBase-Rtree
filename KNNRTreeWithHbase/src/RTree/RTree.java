...
public class RTree {
       ...
       List<String> cuisine_array = new ArrayList<String>();
       List<Double> rate_array = new ArrayList<Double>();
...
// 	           //System.out.println(idxAttrbutes.get(i));
       ...
       public LeafNode chooseLeaf (Node start, Rectangle mbr) {
    	   ...
    	   double areaCurr = start.getMbrArray()[bestchoice].getArea();
    	   Rectangle tmp = start.getMbrArray()[bestchoice];
    	   double areaInc = Rectangle.merge(tmp, mbr).getArea() - areaCurr;
    	   
    	   for (int i = 0; i < start.getNum(); i++) {
		    	Rectangle tmptmp = start.getMbrArray()[i];
		    	double tmpArea = tmptmp.getArea();
		    	double tmpAreaInc = Rectangle.merge(tmptmp, mbr).getArea()
		    			         - tmptmp.getArea();
...
//		    oldNode.mbr = En.getMbr();
       ...
       public NonLeafNode chooseNode (Node start, Rectangle mbr) {
		    ...
		    double areaCurr = start.getMbrArray()[bestchoice].getArea();
		    Rectangle tmp = start.getMbrArray()[bestchoice];
		    double areaInc = Rectangle.merge(tmp, mbr).getArea()
		    		      - tmp.getArea();
