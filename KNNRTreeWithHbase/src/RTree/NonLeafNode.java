...
public class NonLeafNode extends Node{
       ...
       public NonLeafNode () {
	    ...
	    public double getMaxPrice () {
	    	...
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (max < childArray[i].getMaxPrice()) {
	    			max = childArray[i].getMaxPrice();
	    		}
	    	}
	    	return max;
	    }
	    ...
	    public double getMinPrice() {
	    	...
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (min > childArray[i].getMinPrice()) {
	    			min = childArray[i].getMinPrice();
	    		}
	    	}
	    	return min;
		}
		...
		public double getMaxLong() {
	    	double max = Double.MIN_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (max < childArray[i].getMaxLong()) {
	    			max = childArray[i].getMaxLong();
	    		}
	    	}
		...
		public double getMinLong() {
			double min = Double.MAX_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (min > childArray[i].getMinLong()) {
	    			min = childArray[i].getMinLong();
	    		}
	    	}
	    	return min;
		}
		...
		public double getMaxLati() {
			double max = Double.MIN_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (max < childArray[i].getMaxLati()) {
	    			max = childArray[i].getMaxLati();
	    		}
	    	}
	    	return max;
		}
		...
		public double getMinLati() {
			double min = Double.MAX_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (min > childArray[i].getMinLati()) {
	    			min = childArray[i].getMinLati();
	    		}
	    	}
	    	return min;
		}
		...
		public double getMaxRate() {
			double max = Double.MIN_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (max < childArray[i].getMaxRate()) {
	    			max = childArray[i].getMaxRate();
	    		}
	    	}
	    	return max;
		}
		...
		public double getMinRate() {
			double min = Double.MAX_VALUE;
	    	for (int i = 0; i < this.getNum(); i++) {
	    		if (min > childArray[i].getMinRate()) {
	    			min = childArray[i].getMinRate();
	    		}
	    	}
	    	return min;
		}
     ...
     public Node PickNext (List<Node> list, Rectangle Nodembr, Rectangle newNodembr) {
     	 ...
     	 for (Node element : list) {
     		  double areaInc1 = Rectangle.merge(element.getMbr(), Nodembr).getArea()
 				         - newNodembr.getArea();
 			  double areaInc2 = Rectangle.merge(element.getMbr(), newNodembr).getArea()
 					         - Nodembr.getArea();
     ...
     public NonLeafNode split (Node node) {
     	   ...
     	   while (list != null && list.size() != 0) {
     		     ...
     		     double areaIncLeaf = Rectangle.merge(next.getMbr(), Nodembr).getArea() 
     		    		           - Nodembr.getArea();
     		     double areaIncNew = Rectangle.merge(next.getMbr(), Nodembr).getArea()
     		    		          - newNodembr.getArea();
